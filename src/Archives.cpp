/* 
    This file was part of lua player plus
    All CRC functions and password suppourt has been removed and some of my code has been added.
*/
...
static int ZitByte(FILE *file, int *pi)
{
    ...
    int err = sce_paf_fread(&c, 1, 1, file);
...
static unsigned long ZipLocateCentralDir(FILE *file)
{
    ...
    if(sce_paf_fseek(file, 0, SEEK_END) != 0)
        return 0;

    usizefile = sce_paf_ftell(file);
    ...
    while(ubackread < umaxback)
    {
        ...
        if(sce_paf_fseek(file, ureadpos, SEEK_SET) != 0)
            break;

        if(sce_paf_fread(buf, (unsigned int)ureadsize, 1, file) != 1)
            break;
...
static int ZitZipFileInfoInternal(Zip* file, zipFileInfo *pfileinfo, zipFileInternalInfo *pfileinfointernal, char *filename, unsigned long filenamebuffersize, void *extrafield, unsigned long extrafieldbuffersize, char *comment, unsigned long commentbuffersize)
{
    ...
    if(sce_paf_fseek(s->file, s->posincentraldir + s->bytebeforezip, SEEK_SET) != 0)
        err = _ZIP_ERRNO;
    ...
    if((err == _ZIP_OK) && (filename != NULL))
    {
        ...
        {
            if(sce_paf_fread(filename, (unsigned int)usizeread, 1, s->file) != 1)
                err = _ZIP_ERRNO;
        }
    ...
    if((err == _ZIP_OK) && (extrafield != NULL))
    {
        ...
        if(lseek != 0)
        {
            if(sce_paf_fseek(s->file, lseek, SEEK_CUR) == 0)
                lseek = 0;
            else
                err = _ZIP_ERRNO;
        }
        ...
        {
            if(sce_paf_fread(extrafield, (unsigned int)usizeread, 1, s->file) != 1)
                err = _ZIP_ERRNO;
        }
    ...
    if((err == _ZIP_OK) && (comment != NULL))
    {
        ...
        {
            if(sce_paf_fread(comment, (unsigned int)usizeread, 1, s->file) != 1)
                err = _ZIP_ERRNO;
        }
...
static int ZipLocateFile(Zip* file, const char *filename, int casesensitive)
{
    ...
    if(sce_paf_strlen(filename) >= _ZIP_MAX_FILENAME_SIZE)
        return _ZIP_PARAM_ERROR;
    ...
    while(err == _ZIP_OK)
    {
        ...
        if(sce_paf_strcmp(currentfilename, filename) == 0)
            return _ZIP_OK;
...
static int ZipCheckCurrentFileCoherencyHeader(_zip *s, unsigned int *pisizevar, unsigned long *poffsetstaticextrafield, unsigned int *psizestaticextrafield)
{
    ...
    *psizestaticextrafield = 0;
    ...
    if(sce_paf_fseek(s->file, s->currentfileinfointernal.currentfileoffset + s->bytebeforezip, SEEK_SET) != 0)
        return _ZIP_ERRNO;
...
static int ZipReadCurrentFile(Zip* file, void* buf, unsigned int len)
{
    ...
    while(pfileinzipreadinfo->stream.avail_out > 0)
    {
        if((pfileinzipreadinfo->stream.avail_in == 0) && (pfileinzipreadinfo->restreadcompressed > 0))
        {
            ...
            if(sce_paf_fseek(pfileinzipreadinfo->file, pfileinzipreadinfo->posinzip + pfileinzipreadinfo->bytebeforezip, SEEK_SET) != 0)
                return _ZIP_ERRNO;

            if(sce_paf_fread(pfileinzipreadinfo->buffer, ureadthis, 1, pfileinzipreadinfo->file) != 1)
                return _ZIP_ERRNO;
...
Zip* ZipOpen(const char *filename)
{
    ...
    file = sce_paf_fopen(filename, "rb");
    ...
    if(sce_paf_fseek(file, centralpos, SEEK_SET) != 0)
        err = _ZIP_ERRNO;
...
int ZipExtractCurrentFile(Zip *zip, int *nopath, const char *password, const char* path)
{
    ...
    if(err != 0)
    {
        sceClibPrintf("error %d with zipfile in ZitCurrentFileInfo\n", err);
        return -1;
    }
    ...
    if(!buffer)
    {
        sceClibPrintf("Error allocating buffer\n");
    ...
    else
    {
        ...
        if(err != _ZIP_OK)
            sceClibPrintf("Error with zipfile in ZipOpenCurrentFile\n");
        
        sce_paf_snprintf(extract_path, sizeof(extract_path), "%s/%s", path, writeFilename);
        *(filenameWithoutPath - 1) = '\0';
        sce_paf_snprintf(dir_gen_fix, sizeof(dir_gen_fix), "%s/%s", path, writeFilename);
        fout = sce_paf_fopen(extract_path, "wb");
        if (fout == NULL) {
        ...
        if(fout == NULL)
            sceClibPrintf("Error opening file %s\n", writeFilename);
        ...
        do
        {
            ...
            if(err > 0)
            {
                ...
                if (remainingSize == 0){
                    sce_paf_fwrite(buffer, 1, buffersize, fout);
                    remainingSize = buffersize;
                    extractBuffer = buffer;
                }else extractBuffer = (int *)extractBuffer+err;
            }

        } while (err > 0);
        
        if (remainingSize != buffersize) sce_paf_fwrite(buffer, 1, buffersize-remainingSize, fout);
        sce_paf_fclose(fout);

        err = ZipCloseCurrentFile(zip);

        if(err != _ZIP_OK)
            sceClibPrintf("Error with zipfile in ZipCloseCurrentFile, %d\n", err);
    }
...
int ZipExtract(Zip* zip, const char *password, const char* path, ProgressBar *progressReporter)
{
    unsigned int i;
    zipGlobalInfo gi;
    sce_paf_memset(&gi, 0, sizeof(zipGlobalInfo));
    int err;
    int nopath = 0;
...
ZipFile* ZipFileRead(Zip* zip, const char *filename, const char *password)
{
    ...
    if(err != 0)
    {
        sceClibPrintf("error %d with zipfile in ZipOpenCurrentFile\n", err);
        FreePatch(zipfile);
        return NULL;
    }
    ...
    if(!zipfile->data)
    {
        sceClibPrintf("error allocating data for zipfile\n");
        FreePatch(zipfile);
        return NULL;
    }
    ...
    while(err > 0)
    {
        ...
        if(err < 0)
        {
            sceClibPrintf("error %d with zipfile in ZipReadCurrentFile\n", err);
            break;
        }
        else
            count += err;
    ...
    if(err == 0)
    {
        ...
        if(err != 0)
        {
            sceClibPrintf("error %d with zipfile in ZipCloseCurrentFile\n", err);
