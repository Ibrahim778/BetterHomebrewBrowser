...
class ZeroTerminatedRamString {
 public:
  ...
  size_t size() const {
    return _str ? sce_paf_strlen(_str) : 0;
  }
...
class JsonString : public SafeBoolIdom<JsonString> {
 public:
  ...
  JsonString(const char* data, Ownership ownership = Linked)
      : _data(data), _size(data ? sce_paf_strlen(data) : 0), _ownership(ownership) {}
...
class TextFormatter {
 public:
  ...
  void writeRaw(const char* s) {
    _writer.write(reinterpret_cast<const uint8_t*>(s), sce_paf_strlen(s));
  }
...
class MsgPackSerializer : public Visitor<size_t> {
 public:
  ...
  size_t visitString(const char* value) {
    return visitString(value, sce_paf_strlen(value));
  }
  size_t visitString(const char* value, size_t n) {
    ARDUINOJSON_ASSERT(value != NULL);
    if (n < 0x20) {
