...
void sendNotification(const char *text, ...)
{
    ...
    charToWchar(param.text, buf, sizeof(buf));
...
int ExportFilePatched(uint32_t *data)
{
    ...
    if(res == 0x80101A09) // Unsuppourted file
    {
        ...
        if(!scePafFileExists(buff)) //Not a file from bhbb 
            return res;
        ...
        {
            SceInt32 result = unZipTo(buff, param.path);
            if(result < 0)
            {
...
{
    scePafRemoveDirRecursive(EXTRACT_PATH);
    scePafRemoveDirRecursive("ux0:temp/new");
    scePafRemoveDirRecursive("ux0:appmeta/new");
    scePafRemoveDirRecursive("ux0:temp/promote");
    scePafRemoveDirRecursive("ux0:temp/game");

    scePafRemoveDirRecursive("ur0:temp/new");
    scePafRemoveDirRecursive("ur0:appmeta/new");
    scePafRemoveDirRecursive("ur0:temp/promote");
    scePafRemoveDirRecursive("ur0:temp/game");
...
SceInt32 TaskThread(SceSize args, void *argp)
{
    while(running)
    {
        ...
        if(ret == SCE_OK && !NotifMgr_currDlCanceled)
        {
            ...
            unZipToWithProgressCB(CBGDL_DL_PATH, currentTask.dest, UnzipProgressCB);
        ...
        else if(NotifMgr_currDlCanceled)
        {
            ...
            sceIoRemove(CBGDL_DL_PATH);
            scePafRemoveDirRecursive(currentTask.dest);
        }
        else
        {
            sceIoRemove(CBGDL_DL_PATH);
